...
export async function notifyPartner(supabase, userId, action, pactTitle) {
  try {
    ...
    // Batch notifications into a single insert instead of N sequential calls.
    const rows = partnerships.map((partnership) => {
      const partnerId = partnership.user1_id === userId
        ? partnership.user2_id
        : partnership.user1_id;
      ...
      return {
        user_id: partnerId,
        type,
        title: `Partner ${verb} a pact`,
        message: `${name} ${verb}: "${pactTitle}"`,
        metadata: { userId, pactTitle },
      };
    });
    ...
    if (rows.length > 0) {
      const { error: insertErr } = await supabase
        .from('notifications')
        .insert(rows);
      if (insertErr) console.error('Notification batch insert failed:', insertErr);
    }
  } catch (err) {
