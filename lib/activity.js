...
/**
 * Fetch profiles for a list of user IDs and return a map of id -> profile
 * @param {object} supabase - Supabase client
 * @param {string[]} userIds - Array of user IDs
 * @returns {Promise<object>} Map of user ID to profile { id, full_name, avatar_url }
 */
...
export async function logActivity(supabase, action, groupId, metadata = {}) {
  try {
    ...
    if (metadata && typeof metadata !== 'object') {
      console.warn('logActivity: metadata must be an object, got', typeof metadata);
      metadata = {};
    }
...
export async function getGroupActivity(supabase, groupId, limit = 15, offset = 0) {
  try {
    const { data, error } = await supabase
      .from('activity_log')
      .select('id, user_id, group_id, action, metadata, created_at')
      .eq('group_id', groupId)
    ...
    const batchResult = await getBatchReactions(supabase, activityIds);
    const reactionsMap = batchResult?.reactionsMap || {};
...
/**
 * Activity types that are hidden from the main dashboard feed.
 * Deletions are low-signal noise; users care about creation and completion.
 */
export const HIDDEN_FEED_ACTIONS = ['task_deleted'];

/**
 * Patterns in activity metadata that indicate test/debug data.
 * Entries whose metadata.title matches any pattern are filtered out.
 */
export const TEST_DATA_PATTERNS = [/Bulk Test/i, /Test #\d+/i, /\[TEST\]/i];

/**
 * Returns true if an activity entry looks like test/debug data.
 * @param {object} activity - Activity row from activity_log
 * @returns {boolean}
 */
function isTestActivity(activity) {
  const title = activity.metadata?.title || activity.metadata?.name || '';
  return TEST_DATA_PATTERNS.some(pattern => pattern.test(title));
}
...
export async function getAllActivity(supabase, limit = 20, offset = 0) {
  try {
    // Fetch extra rows to compensate for client-side test-data filtering
    const fetchLimit = limit + 10;

    const { data, error } = await supabase
      .from('activity_log')
      .select('id, user_id, group_id, action, metadata, created_at')
      .not('action', 'in', `(${HIDDEN_FEED_ACTIONS.join(',')})`)
      .order('created_at', { ascending: false })
    ...
    // Filter out test/debug entries and trim to the originally requested limit
    const filtered = activitiesWithReactions
      .filter(activity => !isTestActivity(activity))
      .slice(0, limit);
...
export async function getGroupStats(supabase, groupId) {
  try {
    ...
    const profilesMap = await fetchProfilesMap(supabase, memberIds);
