...
export async function awardXP(supabase, userId, eventType, xpAmount, metadata = {}) {
  try {
    ...
    // Broadcast so XPBar/Sidebar/MobileNav refresh without a page reload
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('xp-updated', { detail: { eventType, xpAmount } }));
    }

    return { success: true };
  } catch (err) {
    // Surface full PG error context so the gamification-column trigger and
    // RLS failures are distinguishable in production logs.
    console.error('[awardXP] Failed', {
      eventType,
      xpAmount,
      userId,
      code: err?.code,
  ...
  try {
    ...
    if (achievement) {
      ...
      if (notifResult?.error) {
        console.error('Failed to create achievement notification:', notifResult.error);
      }
    }
...
export async function checkPactAchievements(supabase, userId, totalCompleted, currentStreak) {
  ...
  // Collect all eligible achievement unlocks and run in parallel
  const unlocks = [];

  for (const check of checks) {
    if (totalCompleted >= check.count) {
      unlocks.push(unlockAchievement(supabase, userId, check.key));
    }
  }

  // Streak achievements
  if (currentStreak >= 7) unlocks.push(unlockAchievement(supabase, userId, 'streak_7'));
  if (currentStreak >= 30) unlocks.push(unlockAchievement(supabase, userId, 'streak_30'));
