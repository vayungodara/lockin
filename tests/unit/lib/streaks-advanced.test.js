...
describe('awardStreakFreeze', () => {
  ...
  it('returns capped=true when already at max freezes (5)', async () => {
    const { supabase, builder } = createMockSupabase();
  ...
  it('awards a freeze via RPC when below cap', async () => {
    const { supabase, builder } = createMockSupabase();
  ...
  it('falls back to calculated remaining when RPC returns no data', async () => {
    const { supabase, builder } = createMockSupabase();
    builder.mockReturnValue({ data: { streak_freezes_remaining: 3 }, error: null });
