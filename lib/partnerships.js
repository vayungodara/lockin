...
// UUID validation to safely guard .or() filter interpolation below.
// Supabase's PostgREST .or() uses a comma-separated filter string, so any
// value interpolated into it must be constrained. UUIDs are strictly
// hex+dashes — once validated they're safe to template-literal into the
// filter string.
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function assertUuid(id, name = 'id') {
  if (typeof id !== 'string' || !UUID_RE.test(id)) {
    throw new Error(`Invalid ${name}: not a UUID`);
  }
}
...
export async function sendPartnerRequest(supabase, targetUserId) {
  try {
    ...
    if (authError || !userId) return { success: false, error: authError?.message || 'Not authenticated' };
    ...
    const { data: profile, error: profileError } = await supabase
      ...
      .eq('id', userId)
      .single();
    if (profileError) console.warn('Profile fetch failed:', profileError);
    ...
    if (notifResult?.error) console.error('Notification failed:', notifResult.error);
...
export async function acceptPartnerRequest(supabase, partnershipId) {
  try {
    ...
    // Only the recipient (user2_id) can accept a request
    const { data, error } = await supabase
      .from('accountability_partnerships')
    ...
    if (error) throw error;
    if (!data) return { success: false, error: 'This request is no longer pending' };
...
export async function declinePartnerRequest(supabase, partnershipId) {
  try {
    const { data: authData, error: authError } = await supabase.auth.getUser();
    const userId = authData?.user?.id;
    if (authError || !userId) return { success: false, error: 'Not authenticated' };
...
export async function notifyPartner(supabase, userId, action, pactTitle) {
  try {
    // Verify caller is authenticated before sending notifications
    const { data: authData, error: authError } = await supabase.auth.getUser();
    ...
    // Collect the recipient IDs for each active partnership.
    const recipients = partnerships.map((partnership) =>
      partnership.user1_id === userId ? partnership.user2_id : partnership.user1_id
    );
    if (recipients.length === 0) return;

    // Route through the `notify_partner` RPC (SECURITY DEFINER) which validates
    // the active partnership server-side and bypasses the notifications RLS
    // `WITH CHECK(auth.uid() = user_id)` policy that would otherwise reject a
    // direct batch insert for non-self user_ids.
    const { error } = await supabase.rpc('notify_partner', {
      p_recipients: recipients,
      p_type: type,
      p_title: `Partner ${verb} a pact`,
      p_message: `${name} ${verb}: "${pactTitle}"`,
      p_metadata: { userId, action, pactTitle },
    });

    if (error) console.warn('Notify partner failed:', error.message);
  } catch (err) {
