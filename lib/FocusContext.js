...
export function FocusProvider({ children }) {
  ...
  const endFocusSession = useCallback(async (endTime) => {
    try {
      ...
      // Award XP for completed focus session. Re-fetch user to stay resilient
      // if the provider is long-lived across sign-outs; award is best-effort
      // and must not block the timer flow.
      try {
        const { data: authData } = await supabase.auth.getUser();
        const userId = authData?.user?.id;
        if (userId) {
          const { awardXP, XP_REWARDS } = await import('@/lib/gamification');
          await awardXP(supabase, userId, 'focus_session_completed', XP_REWARDS.FOCUS_SESSION_COMPLETED);
        }
      } catch (err) {
        console.error('Focus session XP award error:', err);
      }
    } catch (err) {
  ...
  // Keep the ref in sync via useEffect so the write happens post-commit.
  // Mutating a ref during render is unsafe under React 18 concurrent mode —
  // render can be discarded, which would leave the ref pointing at a
  // callback that was never committed. The useEffect guarantees the ref
  // only updates after a successful commit.
  useEffect(() => {
  ...
  // A session is "paused mid-flight" when it's stopped but the clock has
  // moved off its full duration (and hasn't hit zero). The UI uses this to
  // hold the in-progress stamp/pulse/caption instead of falling back to the
  // idle render. Distinct from "fresh" (timeLeft === full) and "done"
  // (timeLeft === 0). Applies per mode so a paused break reads as paused too.
  const isPaused = !isRunning && timeLeft > 0 && timeLeft < totalDuration;
  ...
  }), [timeLeft, isRunning, isPaused, mode, sessionsCompleted, progress, formatTime, toggleTimer, resetTimer, skipToNext, switchMode, currentWorkDuration, currentBreakDuration, currentLongBreakDuration]);
