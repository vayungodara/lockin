...
export async function updateStreakOnCompletion(supabase, userId, currentStreak, calculatedLongestStreak, timezone = 'UTC') {
  try {
    ...
    // xp_events row with a distinctive event_type that includes the
    // streak's START date, so a future streak of the same length earns
    // its own freeze (markers don't bleed across streak runs).
    if (FREEZE_REWARD_MILESTONES.includes(currentStreak)) {
      // Streak start date = today - (currentStreak - 1) days. Encode as YYYY-MM-DD.
      const todayMs = Date.now();
      const startMs = todayMs - (currentStreak - 1) * 86400000;
      const startDate = new Date(startMs).toISOString().slice(0, 10);
      const eventType = `streak_freeze_milestone_${currentStreak}_${startDate}`;
