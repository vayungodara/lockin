...
// Editorial-ink celebration palette (default / pact-completion). Highlighter
// yellow is the brand lead, moss green carries the KEPT energy, amber + gold add
// warmth, ink-dark is the "ink on paper" contrast fleck. Hex literals trace to
// the --stamp-* / --ink-* OKLCH tokens in globals.css (brightened a touch so
// particles pop against cream). No retired indigo/purple/magenta on this path.
const BRAND_COLORS = ['#FFDC4C', '#F6D56B', '#399E43', '#F7A224', '#D59800', '#1B1510'];

// Milestone / level-up burst (fireMilestoneConfetti only). Gold-led so the
// dominant read is "golden milestone"; the legacy indigo→purple→magenta trio
// rides the tail as the sanctioned level-up easter-egg nod (.impeccable.md →
// "Brand gradient retired" → level-up celebration carve-out).
const GOLD_COLORS = ['#FFDC4C', '#F7A224', '#D59800', '#6366F1', '#8B5CF6', '#D946EF'];
