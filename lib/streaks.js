...
export async function calculateStreak(supabase, userId, timezone = 'UTC') {
  try {
    // Limit to last 365 days — sufficient for any practical streak calculation
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 365);

    const [{ data: pacts, error }, { count: totalCompleted, error: countError }] = await Promise.all([
      supabase
        .from('pacts')
    ...
    if (!pacts || pacts.length === 0) return { currentStreak: 0, longestStreak: 0, totalCompleted: totalCompleted || 0 };
    ...
    return {
      currentStreak,
      longestStreak,
      totalCompleted: totalCompleted || pacts.length
    };
  } catch (err) {
