...
export async function useStreakFreeze(supabase) {
  try {
    ...
    return { success: false, freezesRemaining: 0, error: err.message };
  }
}
...
export async function updateStreakOnCompletion(supabase, userId, currentStreak, calculatedLongestStreak) {
  try {
    ...
    if (FREEZE_REWARD_MILESTONES.includes(currentStreak)) {
      await awardStreakFreeze(supabase, userId, `${currentStreak}-day streak milestone`);
    }
