# DRAFT v2: `lockin-frontend` Agent — Proposed Prompt

**Purpose:** Proposed content for `.claude/agents/lockin-frontend.md`. Not yet live. Sharing here first so other agents can double-check accuracy against CLAUDE.md, `.impeccable.md`, and the actual codebase before I apply it.

**Revision 2 (2026-04-19):** Rewritten after the editorial-stamps redesign locked `.impeccable.md`. Major changes from v1: fonts (Inter+Instrument Sans → Besley+Host Grotesk+JetBrains Mono), brand color (indigo-purple gradient → flat highlighter yellow), voice (generic second-person → surface-scoped confrontational-wry/direct-positive), component vocabulary (added Stamp/SectionHeader/WitnessTile/StatusChip/NavMarker/NavbarLanding), urgency model (4-state stack → single-signal stamp), color system (hex → OKLCH), + courthouse-vocabulary ban.

**What to check:**
- Do the hard constraints match reality? (stack, conventions, existing patterns)
- Are the component names + file paths correct?
- Does the "First steps" section reference real memory files?
- Are the voice rules accurately summarized from `.impeccable.md`?
- Anything over-engineered or redundant that should be trimmed?

**How to give feedback:** Reply with specific line-range critiques. Inaccuracies flagged > stylistic preferences.

---

## Proposed agent file contents

Everything below this line would go into `.claude/agents/lockin-frontend.md`.
...
- **Target length:** ~195 lines of prompt content (up from v1's ~180 — the redesign added surface-scoped voice rules, ink cascade, single-signal urgency, new components, new anti-patterns)
- **Model choice:** `claude-opus-4-7` (current latest Opus). Memory rule was "never downgrade" — upgrading from 4.6 to 4.7 honors the intent.
- **Tool choice:** Read/Edit/Write/Bash/Grep/Glob + TodoWrite + WebFetch + 8 Playwright MCP tools (includes `browser_hover` for testing `NavMarker` and other hover interactions the agent will implement)
- **Intentional omissions:** No JSON status protocol, no context-manager handoffs, no 7-phase review methodology inline (defer to `/impeccable audit`), no multi-framework coverage, no MCP deps we don't have

## Open questions for review

1. Memory paths correct? `~/.claude/projects/-Users-vayun-projects-lockin/memory/*`
2. All 7 Playwright tool names accurate?
3. "When to hand off" aligned with current Agent Policy in CLAUDE.md?
4. Any redesign-era rule from `.impeccable.md` missing? (Ink cascade, single-signal urgency, voice split, courthouse ban, font lock all captured — am I missing one?)
5. Is `claude-opus-4-6` the correct model ID for frontmatter? (or should it be `opus`?)
