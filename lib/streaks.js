...
export async function calculateStreak(supabase, userId) {
  try {
    ...
    // Use UTC dates for consistent streak calculation across client/server
    const completedDates = [...new Set(
      pacts.map(p => formatUTCDate(new Date(p.completed_at)))
...
export async function getActivityHeatmap(supabase, userId, days = 365) {
  try {
    ...
    // Heatmap uses local dates for display so calendar matches user's day
    (pacts || []).forEach(p => {
      if (p.completed_at) {
